...
6. `_build_full_context()` - 26 edges
7. `generate_full_report()` - 24 edges
8. `build_affiliate_tree()` - 21 edges
9. `get_connection()` - 20 edges
10. `_template_full_report()` - 20 edges
...
- `test_map_info_ignores_director_dict_blob()` --calls--> `map_info_data()`  [INFERRED]
  tests/test_adata_info_mapper.py → app/services/adata/info_mapper.py
- `test_map_info_falls_back_to_diagram_head_name()` --calls--> `map_info_data()`  [INFERRED]
  tests/test_adata_info_mapper.py → app/services/adata/info_mapper.py
...
Cohesion: 0.15
Nodes (24): _collect_bool_flags(), _collect_risk_flags(), _collect_status_flags(), _founders_from_block(), _has_critical_risk(), map_info_data(), _normalize_bin(), _parse_affiliates() (+16 more)
...
Cohesion: 0.11
Nodes (22): _build_courts_verdict(), _call_llm_section(), _combine_sectional_report(), _format_courts_section(), generate_full_report(), _normalize_section_output(), Generate and save full compliance report. Returns report text., Generate and save full compliance report. Returns report text. (+14 more)
...
Cohesion: 0.17
Nodes (14): run(), handleDownloadReport(), caseReportPdfUrl(), downloadCaseReport(), fetchAiStatus(), fetchCase(), fetchCaseScore(), fetchNodeReport() (+6 more)
...
Cohesion: 0.15
Nodes (16): fetch_company_info(), Start company info job and poll until ``data`` is ready; return that object., Start company info job and poll until ``data`` is ready; return that object., Start company info job and poll until ``data`` is ready; return that object., Start company info job and poll until ``data`` is ready; return that object., delete_cached(), get_cached(), _get_client() (+8 more)
...
Cohesion: 0.13
Nodes (16): _build_full_context(), _format_affiliate_analysis(), _format_court_case_line(), _format_director_companies(), _is_low_risk_affiliate(), _log_section(), Build compact but complete context for LLM from all case data., Build compact but complete context for LLM from all case data. (+8 more)
...
Cohesion: 0.13
Nodes (15): _collect_affiliate_enrichments(), _format_affiliate_tree_compact(), Walk affiliate tree, load enriched_data for nodes with hasReport=True., Walk affiliate tree, load enriched_data for nodes with hasReport=True., Walk affiliate tree, load enriched_data for nodes with hasReport=True., Walk affiliate tree, load enriched_data for nodes with hasReport=True., Walk affiliate tree, load enriched_data for nodes with hasReport=True., Walk affiliate tree, load enriched_data for nodes with hasReport=True. (+7 more)
...
Cohesion: 0.15
Nodes (13): _format_individual_court_case(), _is_populated(), _list_data_sources(), List exactly which enriched_data blocks are present and non-empty., List exactly which enriched_data blocks are present and non-empty., Format one individual court case with history and document links., Format one individual court case with history and document links., Format one individual court case with history and document links. (+5 more)
...
Cohesion: 0.18
Nodes (12): _affiliate_profile_summary(), _build_affiliate_profile(), process_case(), Adata enrichment only; AI conclusion is queued separately., Adata enrichment + LSEG screening + unified scoring. AI conclusion queued separa, Adata enrichment + LSEG screening + unified scoring. AI conclusion queued separa, Adata enrichment + LSEG screening + unified scoring. AI conclusion queued separa, Adata enrichment + LSEG screening + unified scoring. AI conclusion queued separa (+4 more)

### Community 39 - "Community 39"
Cohesion: 0.17
Nodes (12): Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data only., Generate a structured text report without LLM based on case data., Generate a structured text report without LLM based on case data only. (+4 more)
...
Cohesion: 0.18
Nodes (11): Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc, Apply LSEG screening + re-score to an already-enriched case. Returns True on suc (+3 more)

### Community 43 - "Community 43"
Cohesion: 0.24
Nodes (6): EnrichmentService, default_section_sources(), merge_section_sources(), Fallback when only case-level provider list is stored., Fallback when only case-level provider list is stored., Fallback when only case-level provider list is stored.
...
Cohesion: 0.2
Nodes (10): fetch_trustworthy_plus(), Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., Return trustworthy-plus compliance data for *iin*, with 12-hour Redis cache., adata_key() (+2 more)

### Community 46 - "Community 46"
Cohesion: 0.24
Nodes (8): Prefer ``basic.fullname_director``; never treat risk ``head`` blob as a name., Prefer ``basic.fullname_director``; never treat risk ``head`` blob as a name., _resolve_director_name(), normalize_person_name(), Return a displayable person name; ignore nested risk/JSON blobs., test_normalize_person_name_accepts_plain_string(), test_normalize_person_name_rejects_json_blob(), test_normalize_person_name_rejects_stringified_dict()
...
Cohesion: 0.31
Nodes (8): CompanyData, _endpoint_ok(), infer_section_sources_from_data(), Infer per-section provider from CompanyData.raw and populated fields., test_company_data_courts_enrichment_from_courtcase(), test_courtcase_failure_marks_none(), test_courtcase_failure_marks_stub(), test_parse_courtcase_maps_totals_and_years()
...
Cohesion: 0.22
Nodes (9): Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre, Re-run LSEG batch screening for all non-resident targets including affiliate tre (+1 more)
...
Cohesion: 0.32
Nodes (7): execute_tool(), OpenAI tool definitions and executors for the compliance chat agent., Execute a tool call and return result as JSON string., Search affiliate tree for name match., Extract key fields for tool response (don't dump everything)., _search_in_tree(), _summarize_enrichment()
...
Cohesion: 0.33
Nodes (5): Tests for unified Adata company/info payload (CORE 24/7 shape)., test_map_core_info_structure(), test_map_info_falls_back_to_diagram_head_name(), test_map_info_ignores_director_dict_blob(), test_provider_map_raw_core()
...
Cohesion: 0.5
Nodes (4): Red flags from riskFactor (not OFAC list — those need trustworthy-extended)., Red flags from riskFactor (not OFAC list — those need trustworthy-extended)., Red flags from riskFactor (not OFAC list — those need trustworthy-extended)., _risk_sanctions()

### Community 89 - "Community 89"
Cohesion: 0.5
Nodes (4): _litigation_block(), _litigation_totals(), Prefer block with more court activity (company vs director)., Returns (litigation data, scope, note).     scope: company | director
...
## Knowledge Gaps
- **379 isolated node(s):** `Compatibility shim for legacy imports.`, `HTTP Basic auth against users stored in PostgreSQL.`, `Settings`, `Ensure company API prefix; fix common ``/api`` without ``/company`` typo.`, `Derive Adata *individual* API base from the company base URL.` (+374 more)
...
- **Why does `str` connect `Community 10` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 7`, `Community 8`, `Community 9`, `Community 13`, `Community 16`, `Community 17`, `Community 18`, `Community 21`, `Community 22`, `Community 23`, `Community 27`, `Community 29`, `Community 31`, `Community 32`, `Community 35`, `Community 36`, `Community 38`, `Community 39`, `Community 41`, `Community 45`, `Community 50`, `Community 58`, `Community 59`, `Community 60`, `Community 66`, `Community 69`, `Community 78`, `Community 102`, `Community 103`?**
  _High betweenness centrality (0.336) - this node is a cross-community bridge._
- **Why does `process_case()` connect `Community 38` to `Community 2`, `Community 35`, `Community 68`, `Community 69`, `Community 58`, `Community 7`, `Community 66`, `Community 3`, `Community 10`, `Community 43`, `Community 45`, `Community 78`, `Community 49`, `Community 50`, `Community 18`, `Community 17`, `Community 90`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `cn()` connect `Community 6` to `Community 12`, `Community 25`, `Community 52`, `Community 53`, `Community 56`, `Community 57`, `Community 70`, `Community 71`, `Community 72`, `Community 79`, `Community 84`, `Community 85`, `Community 86`, `Community 91`, `Community 92`, `Community 93`, `Community 94`, `Community 95`, `Community 96`, `Community 97`, `Community 104`, `Community 105`, `Community 106`, `Community 107`, `Community 108`, `Community 109`?**
